...
- (instancetype)init {
    if (self = [super init]) {
        self.frame = CGRectMake(tab_revealMainWidth - kFlowBallSize - kFlowBallMargin, tab_revealMainHeight/2, kFlowBallSize, kFlowBallSize);
...
- (void)pan:(UIPanGestureRecognizer *)ges {
    ...
    if (ges.state == UIGestureRecognizerStateChanged) {
        ...
        CGFloat transitionX = MAX(kFlowBallSize / 2.0, MIN(self.center.x + transitionP.x, tab_revealMainWidth - kFlowBallSize / 2.0));
        CGFloat transitionY = MAX(tab_revealMainWidth / 2.0, MIN(self.center.y + transitionP.y, tab_revealMainHeight - tab_revealMainWidth / 2.0));
        ...
        [UIView animateWithDuration:kFlowBallAnimationDuration animations:^{
            ...
            CGFloat maxX = tab_revealMainWidth - self.frame.size.width - kFlowBallMargin;
            CGFloat minY = kFlowBallMargin;
            CGFloat maxY = tab_revealMainHeight - self.frame.size.width - kFlowBallMargin;
            CGPoint point = CGPointZero;
            
            if (self.center.x < tab_revealMainWidth / 2.0) {
                point.x = minX;

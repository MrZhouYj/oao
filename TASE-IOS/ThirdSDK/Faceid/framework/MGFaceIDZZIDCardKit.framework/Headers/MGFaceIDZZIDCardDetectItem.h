...
//  身份证检测配置参数
@interface MGFaceIDZZIDCardSettingItem : NSObject

///  可信度，判断身份证可信的阈值。默认为0.5，取值范围为[0, 1]
@property (nonatomic, assign) CGFloat confidence;
///  清晰度，判断身份证清晰的阈值。默认为0.5，取值范围为[0, 1]
@property (nonatomic, assign) CGFloat clear;
///  偏移度，判断身份证边框相对于引导框距离的阈值。默认为0.5，取值范围为[0, 1]
@property (nonatomic, assign) CGFloat inbound;
/* 是否忽略光斑影响，默认为NO
 YES为忽略光斑影响，NO为不忽略光斑影响。
 */
@property (nonatomic, assign) BOOL isIgnoreHighlight;
/* 是否忽略阴影影响，默认为NO
 YES为忽略阴影影响，NO为不忽略阴影影响。
 */

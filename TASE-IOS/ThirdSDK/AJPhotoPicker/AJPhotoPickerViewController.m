...
//    navBar.backgroundColor = [UIColor colorWithRed:236.0/255.0 green:66.0/255.0 blue:67.0/255.0 alpha:1.0];
    ...
    [cancelBtn setTitle:CALanguages(@"取消") forState:UIControlStateNormal];
    ...
    [okBtn setTitle:CALanguages(@"确定") forState:UIControlStateNormal];
...
- (void)showNotAllowed {
    ...
    self.titleLabel.text = CALanguages(@"无权限访问相册");
    self.okBtn.hidden = YES;
    UIAlertView *alert;
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 8.0) {
        alert = [[UIAlertView alloc] initWithTitle:CALanguages(@"提示")
                                           message:CALanguages(@"请在iPhone的“设置”-“隐私”-“照片”中，更改")
                                          delegate:nil
                                 cancelButtonTitle:CALanguages(@"确定")
                                 otherButtonTitles:nil, nil];
    } else {
        alert = [[UIAlertView alloc] initWithTitle:CALanguages(@"提示")
                                           message:CALanguages(@"请先允许访问照片")
                                          delegate:self
                                 cancelButtonTitle:CALanguages(@"取消")
                                 otherButtonTitles:CALanguages(@"前往"), nil];
    }
    [alert show];
}

...
//    [tempView setContentMode:sourceView.contentMode];
    ...
    if(isPortrait == true){
        tempRectSize = (CGSize){MainWidth,(height * MainWidth / width) > MainHeight?MainHeight:(height * MainWidth / width)};
    }else{
        if(width > height){
            if(width / height > MainWidth / MainHeight){
                tempRectSize = (CGSize){MainWidth,height * MainWidth / width};
            }else{
                tempRectSize = (CGSize){MainHeight * width / height,MainHeight};
            }
        }else{
            tempRectSize = (CGSize){(width * MainHeight) / height,MainHeight};
        }
    }
...
- (void)photoBrowserWillDismissWithAnimated:(UIImageView *)tempView items:(KNPhotoItems *)items{
    ...
    if([self isOutOfScreen:rect]){
        ...
        CGFloat width  = tempView.image.size.width;
        CGFloat height = tempView.image.size.height;
        CGSize tempRectSize = (CGSize){MainWidth,(height * MainWidth / width) > MainHeight ? MainHeight:(height * MainWidth / width)};
...
- (BOOL)isOutOfScreen:(CGRect)rect{
    if(isPortrait){
        ...
        if(rect.origin.y > MainWidth ||
           rect.origin.y <= - rect.size.height ||
           rect.origin.x > MainHeight ||
           rect.origin.x <= - rect.size.width){
            return true;
        }
    }
...
- (void)layoutCollectionViewAndLayout{
    ...
    [_numView setFrame:(CGRect){{0,y},{MainWidth,25}}];
    [_pageControl setFrame:(CGRect){{0,self.view.height - 50},{MainWidth,30}}];
    [_operationBtn setFrame:(CGRect){{MainWidth - 35 - 15 - x,y},{35,20}}];
...
- (UIImageView *)tempViewFromSourceViewWithCurrentIndex:(NSInteger)currentIndex{
    ...
    if(imageView.image == nil){
        if (items.isVideo == false) {
            imageView.image = [self createImageWithUIColor:PhotoPlaceHolderDefaultColor size:CGSizeMake(MainWidth, MainWidth)];
        }else {
            imageView.image = [self createImageWithUIColor:UIColor.clearColor size:CGSizeMake(MainWidth, MainWidth)];
        }
    }

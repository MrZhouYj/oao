...
- (void)setupSubViews{
    ...
    for (NSInteger i = 0; i < _otherBtnTitlesArr.count; i++) {   
        ...
        [sheetView setFrame:(CGRect){{0,buttonY},{MainWidth,kActionItemHeight}}];
        ...
        line.frame = CGRectMake(0, buttonY, MainWidth, 0.5);
    ...
    [cancelView setFrame:(CGRect){{0,buttonY},{MainWidth,kActionItemHeight}}];
    
    [cancelView setTitle:_cancelBtnTitle?_cancelBtnTitle:@"取消"];
    [_bgView addSubview:cancelView];
    
    _bgView.frame = CGRectMake(0, MainHeight, MainWidth, height);
}

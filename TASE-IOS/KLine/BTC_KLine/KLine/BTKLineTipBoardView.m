...
//    CGContextSetStrokeColorWithColor(context, HexRGB(0x006cdb).CGColor);//线框颜色
...
- (void)showWithTipPoint:(CGPoint)point isFullScreen:(BOOL)isFullScreen{
    ...
    if (isFullScreen) {
        if (point.x > MainHeight/2.0) {
            frame.origin.x = 10+SafeAreaTopHeight;
        }else {
            frame.origin.x = MainHeight-self.width-10-50-SafeAreaBottomHeight-SafeAreaTopHeight; //减去指标宽度
        }
    }else{
        if (point.x > MainWidth/2.0) {
            frame.origin.x = 10;
        }
        else {
            frame.origin.x = MainWidth-self.width-10;
        }
    }
...
- (void)_setup {
    //填充色
    self.strockColor = HexRGB(0x006cdb);
    self.radius = 4.0;
    ...
    self.timeColor = HexRGB(0x006cdb);
    self.openColor = HexRGB(0x006cdb);
    self.closeColor = HexRGB(0x006cdb);
    self.highColor = HexRGB(0x006cdb);
    self.lowColor = HexRGB(0x006cdb);
    self.riseDropColor = HexRGB(0x006cdb);
    self.percentColor = HexRGB(0x006cdb);
    self.volColor = HexRGB(0x006cdb);
...
- (void)drawText {
    ...
    //画字
    NSArray *titles = @[CALanguages(@"时间"),CALanguages(@"开"),CALanguages(@"高"),CALanguages(@"低"),CALanguages(@"收"),CALanguages(@"涨跌额"),CALanguages(@"涨跌幅"),CALanguages(@"成交量")];
    CGFloat padding = 5;
    
    for (int i = 0; i < titles.count; i ++) {
        NSAttributedString *attString = [[NSAttributedString alloc] initWithString:titles[i] attributes:@{NSFontAttributeName:self.font, NSForegroundColorAttributeName:HexRGB(0x006cdb)}];
    ...
    for (int i = 0; i < contents.count; i ++) {
        ...
        CGFloat originY = 5 + i * (padding + self.font.lineHeight);
        CGFloat titleW = [CommonMethod computeTextSizeWithString:contents[i] andFontSize:FONT_REGULAR_SIZE(10)].width;

...
-(void)setOnlyShowTimeLineView:(BOOL)onlyShowTimeLineView{
    _onlyShowTimeLineView = onlyShowTimeLineView;
    self.isSetCloseMA = YES;
    self.kLineMainView.onlyShowTimeLineView = onlyShowTimeLineView;
    if (onlyShowTimeLineView) {
...
- (UIScrollView *)scrollView
{
    if(!_scrollView)
    {
        ...
        //单点手势
        UITapGestureRecognizer * tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(event_tapMethod:)];
...
#pragma mark k线设置offset的位置
    ...
    self.accessoryMAView.targetLineStatus = self.targetLineAccessoryViewStatus;
    [self.accessoryMAView maProfileWithModel:model];

   
    if (self.scrollView.contentSize.width - 47 >= MainWidth&&!self.isDragging) {
...
-(void)setTargetLineAccessoryViewStatus:(Y_StockChartTargetLineStatus)targetLineAccessoryViewStatus{
    _targetLineAccessoryViewStatus = targetLineAccessoryViewStatus;
    
    if(targetLineAccessoryViewStatus == Y_StockChartTargetLineStatusAccessoryClose){
...
-(UILabel *)dateLabel{
    if (!_dateLabel) {
         ...
         [_dateLabel setFont:[UIFont fontWithName:@"Helvetica Neue" size:9.0]];
         [_dateLabel setBackgroundColor:HexRGBA(0x006cdb, 1)];
...
-(UILabel *)priceLabel{
    if (!_priceLabel) {
        ...
        [_priceLabel setTextColor:HexRGB(0x006cdb)];
        _priceLabel.layer.borderColor = HexRGB(0x006cdb).CGColor;
        _priceLabel.layer.borderWidth = 1;
        [_priceLabel setFont:[UIFont fontWithName:@"Helvetica Neue" size:9.0]];
        [_priceLabel setBackgroundColor:[HexRGB(0x006cdb) colorWithAlphaComponent:0.1]];
...
#pragma mark 单点手势执行方法
-(void)event_tapMethod:(UIGestureRecognizer*)tap{
    if (self.onlyShowTimeLineView) {
        return;
    }
    ...
    //更新横线位置
    CGFloat rightYPosition = location.y;
    ...
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:self.currentKLineModel.Date.doubleValue];
    ...
    else if ((rightXPosition + 30) >= (self.scrollView.contentOffset.x + MainWidth) ) {
        if (self.isFullScreen){
            datePositionX = rightXPosition - 30;
        }else{
            datePositionX = self.scrollView.contentOffset.x + MainWidth - 70;
        }
    }
    else {
        datePositionX = rightXPosition - 30;
    ...
    //手指在屏幕上的x坐标
    CGPoint windowLocation = [tap locationInView:self];
   
    if (self.isFullScreen){ //横屏
        if (windowLocation.x > MainHeight/2.0) {
            pricePositionX = self.scrollView.contentOffset.x + MainHeight - 60-100-SafeAreaBottomHeight; //减去指标宽度 右侧价格宽度
        }else {
            pricePositionX = self.scrollView.contentOffset.x + SafeAreaTopHeight; //减去指标宽度 右侧价格宽度; //减去指标宽度
        }
    }else{
        if (windowLocation.x > MainWidth/2.0) {
            NSLog(@"屏幕右边");
            pricePositionX = self.scrollView.contentOffset.x + MainWidth - 60;
        }else {
            NSLog(@"屏幕左边");
...
- (void)event_longPressMethod:(UILongPressGestureRecognizer *)longPress {
    ...
    if(UIGestureRecognizerStateChanged == longPress.state || UIGestureRecognizerStateBegan == longPress.state){
        ...
        if(!self.verticalView){
            ...
            self.gradient.colors = [NSArray arrayWithObjects:
                               (id)HexRGBA(0x006cdb,0.1).CGColor,
                               (id)HexRGBA(0x006cdb,0.4).CGColor,
                               (id)HexRGBA(0x006cdb,0.1).CGColor, nil];
            [self.verticalView.layer addSublayer:self.gradient];
        ...
        if (self.isFullScreen){ //横屏
            if (windowLocation.y > MainHeight/2.0) {
                pricePositionX = self.scrollView.contentOffset.x + MainHeight - 60-50-SafeAreaBottomHeight; //减去指标宽度 右侧价格宽度
            }else {
                pricePositionX = self.scrollView.contentOffset.x + SafeAreaTopHeight; //减去指标宽度 右侧价格宽度; //减去指标宽度
            }
        }else{
...
- (BTKLineTipBoardView *)tipBoard {
    if (!_tipBoard) {
        _tipBoard = [[BTKLineTipBoardView alloc] initWithFrame:CGRectMake(10, 20, 130.0f, 140)];
         ...
         _tipBoard.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0.7];
...
//    }
    ...
    if (!self.onlyShowTimeLineView) {
        ...
        if (self.targetLineAccessoryViewStatus == Y_StockChartTargetLineStatusAccessoryClose) {
            self.volumeViewRatio = 0.28;
        ...
        [self.kLineVolumeView setHidden:YES];
        [self.kLineAccessoryView setHidden:YES];
        [self.volumeView setHidden:YES];
        [self.volumeMAView setHidden:YES];
...
- (void)kLineMainViewCurrentNeedDrawKLineColors:(NSArray *)kLineColors
{
    ...
    {
        self.kLineAccessoryView.targetLineAccessoryViewStatus = self.targetLineAccessoryViewStatus;
    }
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    if (self.scrollView.contentOffset.x+self.scrollView.width+50>=self.scrollView.contentSize.width) {
        self.isDragging = NO;
        self.kLineMainView.isNeedAutoScroll = YES;
    }else{
        self.kLineMainView.isNeedAutoScroll = NO;
    }
}

-(void)setIsDragging:(BOOL)isDragging{
    _isDragging = isDragging;
    self.kLineMainView.isDragging = isDragging;
}

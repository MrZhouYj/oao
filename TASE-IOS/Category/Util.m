...
+ (CGFloat)caculateTextWidth:(NSString *)text font:(UIFont *)font sizeHeight:(CGFloat)height
{
    ...
    if ([text respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
        NSDictionary *attributes = @{NSFontAttributeName: font};
        size = [text boundingRectWithSize:CGSizeMake(MainWidth, height) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
    }
    else {
        size = [text boundingRectWithSize:CGSizeMake(MainWidth, height) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:font} context:nil].size;
    }
    return size.width;
}
